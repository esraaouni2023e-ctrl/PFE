...
<head>
    ...
    <style>
        ...
        *, *::before, *::after {
        ...
        .bg-orb-1 { width: 300px; height: 300px; top: -100px; right: -50px; background: radial-gradient(circle, color-mix(in srgb,var(--red) 12%,transparent) 0%, transparent 70%); }
...
<body>
    ...
    <header class="mob-header">
        <a href="{{ route('admin.dashboard') }}" class="logo-mob">
            <img src="{{ asset('final.png') }}" alt="Logo">
            <span>CapAvenir <small style="font-size: 0.6rem; background: var(--red); color: #fff; padding: 2px 6px; border-radius: 4px; margin-left: 4px;">Admin</small></span>
        </a>
        <div class="header-actions">
            @php $notifCount = \App\Models\Contact::nonLus()->count(); @endphp
            <a href="{{ route('admin.contacts.index') }}" class="header-btn" title="Messages non lus">
                <i class="bi bi-bell"></i>
                @if($notifCount > 0)
                    <span class="notif-badge-mob">{{ $notifCount }}</span>
                @endif
            </a>
    ...
    <div class="drawer" id="sideDrawer">
        ...
        <div class="drawer-content">
            ...
            <div class="drawer-section">Filtres Utilisateurs</div>
            <a href="{{ route('admin.users.index', ['role' => 'student']) }}" class="drawer-link {{ request()->routeIs('admin.users.*') && request()->get('role') === 'student' ? 'active' : '' }}">
                <i class="bi bi-mortarboard-fill"></i> Étudiants & Profils
            </a>

            <div class="drawer-section">Analytique & Config</div>
            <a href="{{ route('admin.riasec.dashboard') }}" class="drawer-link {{ request()->routeIs('admin.riasec.*') ? 'active' : '' }}">
                <i class="bi bi-cpu-fill"></i> Statistiques IA
            </a>
            <a href="{{ route('admin.references.index') }}" class="drawer-link {{ request()->routeIs('admin.references.*') || request()->routeIs('admin.filieres.*') ? 'active' : '' }}">
                <i class="bi bi-sliders"></i> Paramètres Référentiel
            </a>
            <a href="{{ route('admin.audit.index') }}" class="drawer-link {{ request()->routeIs('admin.audit.*') ? 'active' : '' }}">
                <i class="bi bi-shield-check"></i> Logs d'audit
            </a>
    ...
    <nav class="bottom-nav">
        ...
        <a href="{{ route('admin.users.index') }}" class="tab-item {{ request()->routeIs('admin.users.index') && !request()->has('role') ? 'active' : '' }}">
            <i class="bi bi-people"></i>
            <span>Utilisateurs</span>
        </a>
        <a href="{{ route('admin.counselors.index') }}" class="tab-item {{ request()->routeIs('admin.counselors.*') ? 'active' : '' }}">
            <i class="bi bi-person-badge"></i>
            <span>Conseillers</span>
        </a>
        <a href="{{ route('admin.testimonials.index') }}" class="tab-item {{ request()->routeIs('admin.testimonials.*') ? 'active' : '' }}">
